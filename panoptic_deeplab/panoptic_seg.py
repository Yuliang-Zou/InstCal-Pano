...
# Custom
from .cae import CAE, get_cae_weights
...
@META_ARCH_REGISTRY.register()
class DeepAugmentPanopticDeepLab(PanopticDeepLab):
    """
    Main class for panoptic segmentation architectures using DeepAugment before feeding into the model.
    """

    def __init__(self, cfg):
        super().__init__(cfg)
        self.augment_net = CAE()
        model_path = 'pretrained_model/cae_model.state'
        self.augment_net.load_state_dict(get_cae_weights(model_path))
        self.augment_net.eval()
        self.augment_net.to(self.device)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            self.augment_net.eval()
            with torch.no_grad():
                images = [self.augment_net(x.unsqueeze(0)/255.)[0]*255. for x in images]
