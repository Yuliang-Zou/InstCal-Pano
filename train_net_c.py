...
from detectron2.projects.panoptic_deeplab import InstCalC
...
def set_norm_requires_grad(module, flag=True):
    for module in module.modules():
        for name, param in module.named_parameters():
            if 'momentum' in name or 'mean_net' in name or 'var_net' in name:
...
class Trainer(DefaultTrainer):
    ...
    def __init__(self, cfg):
        ...
        self.checkpointer.model = InstCalC.convert_conditional_batchnorm(self.checkpointer.model)
...
def main(args):
    ...
    if args.eval_only:
        ...
        model = InstCalC.convert_conditional_batchnorm(model)
        model.cuda()
